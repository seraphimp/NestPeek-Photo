    ...
    <div class="profile-inner">
        ...
        <div class="profile-actions">
            ...
            <div style="display:flex;gap:8px;align-items:center">
                ...
                @endif
                {{-- ── FIX: Book Now only for clients ── --}}
                @if($isClient)
                <button class="btn btn-primary" onclick="scrollToBook()">Book Now</button>
                @elseif(!auth()->check())
                <a href="{{ route('login') }}" class="btn btn-primary">Sign in to Book</a>
                @endif
            </div>
        </div>
    </div>
...
<div class="page-body">
    ...
    <div>
        ...
        <div class="section">
            ...
            @php
            $allMedia = $creatorProfile->portfolios->flatMap(fn($p) => $p->media)->filter(fn($m) => $m->type === 'image');
            @endphp
        ...
        <div class="section">
            ...
            @empty
            <p style="font-family:'Inter',sans-serif;font-size:0.85rem;color:var(--slate-mid);font-style:italic;padding:16px 0">No services listed yet.</p>
            @endforelse
        </div>
        ...
        <div class="section">
            ...
            <div class="tag-list" style="margin-bottom:14px">
                @foreach($creatorProfile->styles as $style)<span class="tag">{{ $style }}</span>@endforeach
            </div>
            @endif
            ...
            <div class="tag-list" style="margin-bottom:14px">
                @foreach($creatorProfile->skills as $skill)<span class="tag">{{ $skill }}</span>@endforeach
            </div>
            @endif
            ...
            <div class="tag-list">
                @foreach($creatorProfile->equipment as $item)<span class="tag">{{ $item }}</span>@endforeach
            </div>
            @endif
        </div>
        @endif
        ...
        <div class="section">
            ...
            {{-- ── FIX: Review form — clients with a completed unreviewed booking only ── --}}
            @if($isClient && $completedBooking)
            <div class="review-form-wrap">
                <p class="review-form-title">Leave a Review</p>
                <p class="review-form-sub">
                    Your experience from your booking on {{ \Carbon\Carbon::parse($completedBooking->event_date)->format('M j, Y') }}
                </p>

                @if($errors->has('rating') || $errors->has('body'))
                <div style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.25);border-radius:10px;padding:10px 14px;margin-bottom:14px;font-family:'Inter',sans-serif;font-size:0.8rem;color:#991B1B;">
                    @foreach($errors->only(['rating','body']) as $err)<p>{{ $err }}</p>@endforeach
                </div>
                @endif

                <form method="POST" action="{{ route('reviews.store', $completedBooking) }}">
                    @csrf

                    {{-- Star rating — pure CSS, RTL trick so hover highlights correctly --}}
                    <div style="margin-bottom:16px;">
                        <p style="font-family:'DM Mono',monospace;font-size:0.6rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--slate-mid);margin-bottom:8px;">Your Rating <span style="color:#EF4444">*</span></p>
                        <div class="star-row" id="stars">
                            @for($s = 5; $s >= 1; $s--)
                            <input type="radio" name="rating" id="star{{ $s }}" value="{{ $s }}" {{ old('rating') == $s ? 'checked' : '' }} required>
                    ...
                    <div style="margin-bottom:12px;">
                        <p style="font-family:'DM Mono',monospace;font-size:0.6rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--slate-mid);margin-bottom:8px;">Title <span style="color:var(--slate-mid)">(optional)</span></p>
                        <input type="text" name="title" value="{{ old('title') }}"
                            placeholder="e.g. Amazing wedding photographer!"
                            style="width:100%;background:var(--sky-faint);border:1.5px solid var(--border);border-radius:12px;padding:10px 14px;font-family:'Inter',sans-serif;font-size:0.85rem;color:var(--text-main);outline:none;transition:border-color 0.15s;"
                            onfocus="this.style.borderColor='var(--sky-mid)'"
                            onblur="this.style.borderColor='var(--border)'">
                    </div>

                    {{-- Review body --}}
                    <div style="margin-bottom:4px;">
                        <p style="font-family:'DM Mono',monospace;font-size:0.6rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--slate-mid);margin-bottom:8px;">Your Review <span style="color:#EF4444">*</span></p>
                        <textarea name="body" class="review-textarea" required
                            placeholder="What was it like working with {{ $creatorProfile->display_name }}? Share the details of your experience...">{{ old('body') }}</textarea>
                    </div>

                    <button type="submit" class="review-submit">Submit Review ★</button>
                </form>
            </div>
            @elseif($isClient && !$completedBooking)
            <div style="background:var(--sky-faint);border:1.5px solid var(--border);border-radius:12px;padding:14px 18px;margin-bottom:16px;font-family:'Inter',sans-serif;font-size:0.82rem;color:var(--slate-mid);">
                💬 You can leave a review after completing a booking with this creator.
            </div>
            @endif
            {{-- Guests and other creators see nothing above ↑ --}}
            ...
            <div class="review-card">
                ...
                <div class="review-stars">
                    @for($i = 1; $i <= 5; $i++){{ $i <= $review->rating ? '★' : '☆' }}@endfor
                        ...
                        @endif
                        <p class="review-body">{{ $review->body ?? $review->content }}</p>
                </div>
                @empty
        ...
        <div>
            <div class="book-cta" id="book">
                ...
                @else
                {{-- logged-in creator viewing another creator's profile --}}
                <p style="color:rgba(255,255,255,0.7);font-size:0.8rem;margin:0">Switch to a client account to book.</p>
                @endif
            </div>
            ...
            <div class="sidebar-card">
                ...
                @if($creatorProfile->user->location)
                <div class="sidebar-row"><span class="sidebar-row-icon">📍</span><span>{{ $creatorProfile->user->location }}</span></div>
                @endif
                @if($creatorProfile->service_areas)
                <div class="sidebar-row"><span class="sidebar-row-icon">🗺️</span><span>Covers: {{ implode(', ', $creatorProfile->service_areas) }}</span></div>
                @endif
                @if($creatorProfile->years_experience > 0)
                <div class="sidebar-row"><span class="sidebar-row-icon">🏅</span><span>{{ $creatorProfile->years_experience }} years experience</span></div>
                @endif
                @if($creatorProfile->user->instagram || $creatorProfile->user->facebook)
                <div class="sidebar-social">
                    @if($creatorProfile->user->instagram)
                    <a href="https://instagram.com/{{ ltrim($creatorProfile->user->instagram,'@') }}" target="_blank" class="social-link">Instagram</a>
                    @endif
                    @if($creatorProfile->user->facebook)
                    <a href="{{ $creatorProfile->user->facebook }}" target="_blank" class="social-link">Facebook</a>
                    @endif
                </div>
                @endif
            </div>
    ...
    <script>
        ...
        {
            {
                --──FIX: favorite toggle— correct favorable_type, clients only──--
            }
        }
        @if($isClient)
        async function toggleFavorite(creatorId) {
            ...
            try {
                const res = await fetch('/favorites/toggle', {
                    ...
                    body: JSON.stringify({
                        favorable_type: 'App\\Models\\CreatorProfile', // ← was 'creator', now full class name
