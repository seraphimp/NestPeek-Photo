...
@section('title', 'Create Studio — NestPeek Photo')
...
<div class="form-page">
    <div class="form-header">
        <h1 class="form-title">Create Your Studio</h1>
        <p class="form-sub">Set up your studio profile so clients can find and book you.</p>
    </div>
    ...
    <form method="POST" action="{{ route('studios.store') }}" enctype="multipart/form-data">
        ...
        <div class="form-card">
            @php($studio = new \App\Models\Studio())
            @include('studios._form', ['studio' => $studio])
        </div>

        <div class="form-actions">
            <a href="{{ route('dashboard') }}" class="btn-cancel">Cancel</a>
            <button type="submit" class="btn-submit">Create Studio</button>
        </div>
    </form>
</div>
